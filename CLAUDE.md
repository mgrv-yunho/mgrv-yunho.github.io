...
- 사이트 전체가 **단일 비밀번호**. 한 번 입력하면 remember-me로 모든 글이 열린다.
  - 비밀번호는 **레포에 저장하지 않는다.** 빌드 시 `STATICRYPT_PASSWORD` 환경변수로만 주입한다 — 코드·커밋 메시지·문서 어디에도 적지 않는다.
...
- 날짜·시간: `<meta name="date" content="YYYY-MM-DD HH:MM" />`
  - 시간(`HH:MM`)은 선택 — `YYYY-MM-DD`만 써도 됨. 구분자는 공백 또는 `T` 둘 다 허용.
  - 시간이 있으면 목차에 날짜 밑에 함께 표시되고, 정렬도 분 단위까지 반영된다.
  - date 메타가 아예 없으면 제목=파일명, 날짜+시각=파일 수정시각으로 폴백.
  - 본문에도 날짜·시간을 보이려면 h1 아래에 `<p class="meta">…</p>` 한 줄을 둔다(기존 글 참고).
...
- 실제 비밀번호를 코드/커밋 메시지/채팅/문서에 하드코딩하지 않는다 (예외 없음 — 데모 비번도 레포에 두지 않는다).
